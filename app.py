...
with btn_col2:
    if run_btn:
        st.success("Simulation execution complete.")
    else:
        st.info("Adjust parameters in the sidebar and click 'Start Curing Simulation' to execute the run.")

# Simulate computation time if run button is clicked
if run_btn:
...
with chart_col1:
    ...
    fig_temp.update_layout(
        ...
        yaxis=dict(range=[params.T_amb - 5, max(T_n.max(), T_s.max()) + 5])
    )
    st.plotly_chart(fig_temp, use_container_width=True, key="temp_chart_display")
...
with chart_col2:
    ...
    st.plotly_chart(fig_valve, use_container_width=True, key="valve_chart_display")
...
st.plotly_chart(fig_steam, use_container_width=True, key="steam_chart_display")
